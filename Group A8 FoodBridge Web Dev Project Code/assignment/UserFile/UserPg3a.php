...
$pick_up_location = filter_input(INPUT_POST, 'pick_up_location');
$state = filter_input(INPUT_POST, 'state');
$foodDescription = filter_input(INPUT_POST, 'foodDescription');
$category = filter_input(INPUT_POST, 'category');
$totalWeight = filter_input(INPUT_POST, 'totalWeight');
$collectionTime = filter_input(INPUT_POST, 'collectionTime');
$collectionDate = filter_input(INPUT_POST, 'collectionDate');
    ...
    else
    {
        ...
        die();
            ...
            die();
               ...
               {
               echo " Pick Up Locaiton should not be empty";
               die();
               }   
                  if(!empty($state)) 
                  {
                  ...
                  die();
                     ...
                     {
                     echo " Food Description should not be empty";
                     die();
                     }   
                        if(!empty($category))  
                        {
                        ...
                        die();
                           ...
                           {
                           echo " Total Weight should not be empty";
                           die();
                           }   
                              ...
                              {
                              echo " Collection Time should not be empty";
                              die();
                              }   
                                 ...
                                 {
                                 echo " Collectection Date should not be empty";
                                 die();
                                 }
                                    if(!empty($foodBankLocation))
                                    {
                                        ...
                                        if (mysqli_connect_error()){
                                            ...
                                            $sql = "INSERT INTO fooddonation (firstName, lastName, contact, email, pick_up_location, state, foodDescription, category, totalWeight, collectionTime, collectionDate, foodBankLocation)
                                            values ('$firstName', '$lastName', '$contact', '$email', '$pick_up_location', '$state', '$foodDescription', '$category', '$totalWeight', '$collectionTime', '$collectionDate', '$foodBankLocation')";
                                            if ($conn->query($sql)){
                                                echo "-";
